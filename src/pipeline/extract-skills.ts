...
const DEFAULT_EXECUTION: SkillExecution = {
  target_systems: [],
  write_back_actions: [],
...
const DEFAULT_PROBLEM_STATEMENT: SkillProblemStatement = {
  current_state: "",
  quantified_pain: "",
  root_cause: "",
...
function getCrossFunctionalDomains(skill: Skill): string[] {
  const scope = skill.cross_functional_scope;
  if (scope && typeof scope === "object" && !Array.isArray(scope)) {
    const rawDomains = scope.l1_domains;
    if (Array.isArray(rawDomains)) {
      return rawDomains.filter((domain): domain is string => typeof domain === "string" && domain.length > 0);
    }
  }
  return [];
}

function deriveFinancialRating(maxValue: number): FinancialRating {
  if (maxValue >= 5_000_000) return "HIGH";
  if (maxValue >= 1_000_000) return "MEDIUM";
  return "LOW";
}

function deriveImpactOrder(maxValue: number): ImpactOrder {
  return maxValue >= 5_000_000 ? "FIRST" : "SECOND";
}

function hasProblemSignal(skill: Skill): boolean {
...
function hasExecutionSignal(skill: Skill): boolean {
...
function deriveRatingConfidenceFromSkills(
  skills: Skill[],
  options: {
  ...
  if (options.decisionExists) populatedSignals++;
  if (options.aiSuitability === "HIGH") populatedSignals++;

  if (skills.some((skill) => skill.decision_made || skill.walkthrough_decision || skill.walkthrough_narrative)) {
    populatedSignals++;
  }
  if (skills.some(hasProblemSignal)) populatedSignals++;
  if (skills.some(hasExecutionSignal)) populatedSignals++;
  if (skills.some((skill) => (skill.actions?.length ?? 0) + (skill.constraints?.length ?? 0) > 0)) {
    populatedSignals++;
  }

  if (populatedSignals >= 3) return "HIGH";
  if (populatedSignals >= 1) return "MEDIUM";
  return "LOW";
}

function deriveAiSuitability(skill: Skill): AiSuitability {
...
function toCrossFunctionalL4(skill: Skill): L4Activity {
...
function shouldBackfillEmbeddedConfidence(hierarchy: L4Activity[]): boolean {
  return hierarchy.length > 0 && hierarchy.every((l4) => l4.rating_confidence === "LOW");
}

function normalizeEmbeddedL4(l4: L4Activity, backfillConfidence: boolean): L4Activity {
...
export function buildScoringHierarchy(data: HierarchyExport): L4Activity[] {
  const backfillEmbeddedConfidence = shouldBackfillEmbeddedConfidence(data.hierarchy);
  const normalizedHierarchy = data.hierarchy.map((l4) =>
    normalizeEmbeddedL4(l4, backfillEmbeddedConfidence),
  );
 ...
 */
export function extractScoringSkills(hierarchy: L4Activity[]): SkillWithContext[] {
  const skills: SkillWithContext[] = [];

  for (const l4 of hierarchy) {
    if (!l4.skills || l4.skills.length === 0) continue;
