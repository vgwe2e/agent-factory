...
import type { HierarchyExport, L4Activity, Skill } from "../types/hierarchy.js";
...
function makeL4(l3: string, name: string, skills: Skill[] = [], overrides: Partial<L4Activity> = {}): L4Activity {
  return {
    id: `${l3}-${name}`,
    name,
...
describe("extractScoringSkills", () => {
  ...
  it("adds cross-functional skills as synthetic L4 activities", () => {
    ...
    const data: HierarchyExport = {
      meta: {} as any,
      company_context: {} as any,
      hierarchy,
      l3_opportunities: [],
  ...
  it("backfills embedded confidence when the export collapses all L4s to LOW", () => {
    const richSkill = makeSkill({
      id: "embedded-rich",
      actions: [{ action_type: "notify", action_name: "notify", description: "notify" } as any],
      constraints: [{ constraint_type: "threshold", constraint_name: "threshold", description: "threshold" } as any],
    });
