...
const FINANCIAL_MAP: Record<string, number> = {
  HIGH: 1.0,
  MEDIUM: 0.5,
  LOW: 0.0,
};
...
const AI_MAP: Record<string, number> = {
  HIGH: 1.0,
  MEDIUM: 0.5,
  LOW: 0.25,
  NOT_APPLICABLE: 0.0,
...
export function scoreAiSuitability(l4: L4Activity): number {
  if (l4.ai_suitability === null) return 0.0;
...
export function scoreArchetypeCompleteness(l4: L4Activity): number {
  ...
  for (const skill of l4.skills) {
    ...
    if (skill.execution !== null) {
      if (skill.execution.target_systems.length > 0) totalPopulated++;
      if (skill.execution.execution_trigger !== null) totalPopulated++;
      if (skill.execution.execution_frequency !== null) totalPopulated++;
      if (skill.execution.autonomy_level !== null) totalPopulated++;
      if (skill.execution.approval_required !== null) totalPopulated++;
    }
