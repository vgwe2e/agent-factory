...
describe("buildConsolidatedPrompt", () => {
  ...
  it("should include all 3 negative constraints in system message", () => {
    ...
    const system = messages[0].content;
    assert.ok(
      system.includes("Do NOT score platform_fit >= 2 based on generic keyword overlap alone"),
      "Should include keyword overlap constraint",
    );
    assert.ok(
      system.includes("Do NOT let sanity check override more than 2 dimensions"),
      "Should include sanity override constraint",
    );
    assert.ok(
      system.includes("Do NOT assume all HIGH ai_suitability candidates deserve strong platform fit"),
      "Should include ai_suitability constraint",
    );
  });
  ...
  it("should include DISAGREE flagged_dimensions instruction in system message", () => {
    ...
    const system = messages[0].content;
    assert.ok(
      system.includes("DISAGREE") && system.includes("flagged_dimensions"),
      "Should instruct DISAGREE to cite flagged_dimensions",
    );
  });
