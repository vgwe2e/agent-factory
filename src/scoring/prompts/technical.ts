...
${knowledgeContext}`;
  ...
  if (l4s.length > 8) {
    ...
    l4Summary = l4s
      .map((l4) =>
        `- ${l4.name} | financial_rating=${l4.financial_rating} | decision_exists=${l4.decision_exists} | ai_suitability=${l4.ai_suitability ?? "N/A"} | rating_confidence=${l4.rating_confidence}`,
      )
      .join("\n");
  } else {
