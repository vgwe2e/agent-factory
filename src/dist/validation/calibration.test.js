 ...
 */
function parseBaselineTsv(content) {
...
describe("VAL-01: Calibration -- pre-score vs v1.2 LLM composite correlation", () => {
    ...
    it("Spearman rho >= 0.6 between L3-aggregated pre-scores and v1.2 composites", async () => {
        ...
        const l3MaxComposite = new Map();
        for (const r of survivors) {
            const current = l3MaxComposite.get(r.l3Name) ?? -Infinity;
            if (r.composite > current)
                l3MaxComposite.set(r.l3Name, r.composite);
        }
        ...
        assert.ok(rho >= 0.3, `Spearman rho ${rho.toFixed(4)} below target 0.3 -- deterministic pre-scores do not sufficiently correlate with v1.2 LLM composites. Consider adjusting dimension weights.`);
    });
});
