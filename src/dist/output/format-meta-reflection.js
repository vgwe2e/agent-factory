...
function computeCatalogStats(triaged, scored, simResults) {
    ...
    // Red flag frequency
    const redFlagFrequency = new Map();
    for (const t of triaged) {
...
export function formatMetaReflection(triaged, scored, simResults, date, simSkipped) {
    ...
    if (stats.archetypeDistribution.size === 0) {
        ...
        lines.push("|-----------|-------|------------|");
        const sortedArchetypes = [...stats.archetypeDistribution.entries()].sort((a, b) => b[1] - a[1]);
    ...
    if (stats.redFlagFrequency.size === 0) {
        ...
        lines.push("|-----------|-------|--------------|");
        const sortedFlags = [...stats.redFlagFrequency.entries()].sort((a, b) => b[1] - a[1]);
    ...
    if (stats.archetypeDistribution.size > 0) {
        const topArchetype = [...stats.archetypeDistribution.entries()].sort((a, b) => b[1] - a[1])[0];
        lines.push(`**Most Common Archetype:** ${topArchetype[0]} (${topArchetype[1]} occurrences)`);
        lines.push("");
    }
    if (stats.redFlagFrequency.size > 0) {
        const topFlag = [...stats.redFlagFrequency.entries()].sort((a, b) => b[1] - a[1])[0];
