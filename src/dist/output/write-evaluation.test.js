...
describe("writeEvaluation", () => {
    ...
    it("derives tier1Names from triage data", async () => {
        ...
        assert.ok(!reportMd.includes("## ") || !reportMd.includes("Tier2Item"), "tier1 report should not have Tier2Item as a section");
    });
    it("returns error result if directory creation fails", async () => {
        // Use an invalid path that can't be created
        const result = await writeEvaluation("/dev/null/impossible-path", [makeScoring()], [makeTriage()], "TestCorp", DATE);
