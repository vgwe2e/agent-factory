...
// ---------------------------------------------------------------------------
function makeLensScore(lens, overrides) {
    const configs = {
        technical: {
            subDimensions: [
...
function makeV13ScoringResult(overrides) {
    fixtureCounter++;
    return {
...
function makeV13TriageResult(overrides) {
    fixtureCounter++;
    return {
        l3Name: `Test L3 Opportunity ${fixtureCounter}`,
        l2Name: "Test L2 Area",
...
// ---------------------------------------------------------------------------
function extractTsvColumns(tsv) {
    return tsv.split("\n")[0].split("\t");
}
function countTsvRows(tsv) {
    return tsv.split("\n").filter(l => l.trim()).length - 1; // minus header
}
function assertColumnsPresent(tsv, requiredCols) {
...
function assertConsistentColumnCount(tsv) {
    const lines = tsv.split("\n").filter(l => l.trim());
    const headerCount = lines[0].split("\t").length;
    for (let i = 1; i < lines.length; i++) {
        assert.strictEqual(lines[i].split("\t").length, headerCount, `Row ${i} has ${lines[i].split("\t").length} columns, expected ${headerCount}`);
    }
}
...
function assertSectionsMatch(actual, baseline, allowedAdditions = []) {
    ...
    const filterH1H2 = (sections) => sections.filter(s => /^#{1,2} /.test(s) && !/^### /.test(s))
        .filter(s => !/^## \d+\. /.test(s));
    ...
    for (const section of baselineH1H2) {
        assert.ok(actualH1H2.includes(section), `Missing section: "${section}". Actual sections: ${JSON.stringify(actualH1H2)}`);
    }
    ...
    for (const section of actualH1H2) {
        if (!baselineH1H2.includes(section)) {
            assert.ok(allowedAdditions.some(a => section.includes(a)), `Unexpected section: "${section}"`);
        }
    }
}
// ---------------------------------------------------------------------------
...
describe("VAL-03: TSV formatter structural parity", () => {
    ...
    it("formatTriageTsv: valid TSV with consistent column count", () => {
        const triaged = [
            makeV13TriageResult({ tier: 1, quickWin: true }),
            makeV13TriageResult({ tier: 2 }),
        ];
...
// ---------------------------------------------------------------------------
const BASELINE_DIR = path.resolve(import.meta.dirname ?? path.dirname(new URL(import.meta.url).pathname), "../evaluation-vllm/evaluation");
...
describe("VAL-03: Markdown formatter structural parity", () => {
    ...
    it("formatSummary: section headers match v1.2 baseline", () => {
        ...
        const triaged = [
            makeV13TriageResult({ tier: 1 }),
            makeV13TriageResult({ tier: 2 }),
            makeV13TriageResult({ tier: 3 }),
        ];
        const simResults = makeMinimalSimResult();
    ...
    it("formatSummary: table columns match v1.2 baseline", () => {
        ...
        for (let t = 0; t < baselineTables.length; t++) {
            assert.ok(t < actualTables.length, `Missing table ${t + 1} in actual output. Baseline has ${baselineTables.length} tables, actual has ${actualTables.length}`);
            for (const col of baselineTables[t]) {
                assert.ok(actualTables[t].includes(col), `Table ${t + 1} missing column "${col}". Actual: ${JSON.stringify(actualTables[t])}`);
            }
        }
    });
    ...
    it("formatMetaReflection: table columns match v1.2 baseline", () => {
        ...
        const baselineTables = extractTableHeaders(baseline);
        for (let t = 0; t < baselineTables.length; t++) {
            assert.ok(t < actualTables.length, `Missing table ${t + 1} in meta-reflection. Baseline has ${baselineTables.length} tables, actual has ${actualTables.length}`);
            for (const col of baselineTables[t]) {
                assert.ok(actualTables[t].includes(col), `Meta-reflection table ${t + 1} missing column "${col}". Actual: ${JSON.stringify(actualTables[t])}`);
            }
        }
    });
    ...
    it("formatTier1Report: per-opportunity sub-sections present", () => {
        ...
        for (const required of ["Technical Feasibility", "Adoption Realism", "Value & Efficiency", "Assessment"]) {
            assert.ok(h3Sections.some(s => s.startsWith(required)), `Tier1 report missing sub-section: "${required}". Found: ${JSON.stringify(h3Sections)}`);
        }
    });
    ...
    it("formatAdoptionRisk: table columns match v1.2 baseline", () => {
        ...
        for (const col of baselineTables[0]) {
            assert.ok(actualTables[0].includes(col), `Adoption-risk table missing column "${col}". Actual: ${JSON.stringify(actualTables[0])}`);
        }
    });
});
