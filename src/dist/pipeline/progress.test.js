...
describe("progress tracker", () => {
    it("start/complete updates counts correctly", () => {
        ...
        const tracker = createProgressTracker(5, logger);
        tracker.start("opp-1");
        tracker.start("opp-2");
    ...
    it("error decrements inFlight and increments error count", () => {
        const logger = makeMockLogger();
        const tracker = createProgressTracker(3, logger);
        tracker.start("opp-1");
        tracker.start("opp-2");
        tracker.error("opp-1");
        tracker.report();
    ...
    it("report() includes percentDone and etaSeconds", () => {
        const logger = makeMockLogger();
        const tracker = createProgressTracker(10, logger);
        tracker.start("opp-1");
        tracker.complete("opp-1");
        tracker.report();
        const data = logger.infoCalls[0].data;
    ...
    it("report() logs with 'Pipeline progress' message prefix", () => {
        const logger = makeMockLogger();
        const tracker = createProgressTracker(2, logger);
        tracker.start("opp-1");
        tracker.complete("opp-1");
        tracker.report();
        assert.ok(logger.infoCalls[0].msg.startsWith("Pipeline progress:"), `Expected message to start with 'Pipeline progress:', got: ${logger.infoCalls[0].msg}`);
    });
    it("report() includes in-flight opportunity name and elapsed time", () => {
        ...
        tracker.report();
        const msg = logger.infoCalls[0].msg;
