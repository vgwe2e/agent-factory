...
export async function runPipeline(inputPath, options, logger) {
    ...
    const checkpoint = isStale
        ? { version: 1, inputFile: inputPath, startedAt: new Date().toISOString(), entries: [] }
        : existingCheckpoint;
    const completed = isStale ? new Set() : getCompletedNames(existingCheckpoint);
    if (completed.size > 0) {
    ...
    const skipped = triageResults.filter((t) => t.action === "skip" || t.action === "demote");
    ...
    const modelManager = new ModelManager({ triageModel, scoringModel, timeoutMs: MODEL_TIMEOUT_MS }, logger, options.fetchFn, 0);
