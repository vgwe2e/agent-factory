...
describe("buildKnowledgeContext", () => {
    ...
    it("components string contains at least 21 entries", () => {
        ...
        assert.ok(lines.length >= 21, `Expected at least 21 component entries, got ${lines.length}`);
    });
    it("processBuilder string contains at least 22 entries", () => {
        const lines = ctx.processBuilder.split("\n").filter((l) => l.startsWith("- "));
        assert.ok(lines.length >= 22, `Expected at least 22 PB node entries, got ${lines.length}`);
    });
    ...
    it("total knowledge context is under estimated 4000 tokens", () => {
        ...
        assert.ok(estimatedTokens < 4000, `Estimated ${Math.round(estimatedTokens)} tokens exceeds 4000 budget`);
    });
});
