 ...
 */
export function scaleTo03(value) {
    if (value >= 0.75)
        return 3;
    if (value >= 0.50)
        return 2;
 ...
 */
export function computeTwoPassComposite(preScoreComposite, platformFitNormalized, sanityVerdict) {
    const blended = preScoreComposite * PRE_SCORE_WEIGHT +
        platformFitNormalized * LLM_WEIGHT;
    const penalty = sanityVerdict === "DISAGREE"
        ? DISAGREE_PENALTY
 ...
 */
export function buildTechnicalLensFromLLM(llmOutput) {
    const subDimensions = [
        {
            name: "platform_fit",
 ...
 */
export function buildAdoptionLensFromDeterministic(preScore) {
 ...
 */
export function buildValueLensFromDeterministic(preScore) {
    const dims = preScore.dimensions;
    const subDimensions = [
 ...
 */
export async function scoreConsolidated(skill, knowledgeContext, preScore, chatFn) {
    const startMs = Date.now();
    // Build prompt
    const messages = buildConsolidatedPrompt(skill, knowledgeContext, preScore);
    // Single LLM call with Zod validation + retry
    const result = await scoreWithRetry(ConsolidatedLensSchema, async () => {
    ...
    const { composite, promotedToSimulation } = computeTwoPassComposite(preScore.composite, platformFitNormalized, llmOutput.sanity_verdict);
