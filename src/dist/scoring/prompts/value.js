 ...
 */
export function buildValuePrompt(opp, l4s, company, archetypeHint) {
...
Return JSON with score (integer 0-3) and reason (1-2 concise sentences) for each dimension: value_density, simulation_viability.`;
    const l4ValueSummary = l4s
        .map((l4) => `- ${l4.name} | value_metric: ${l4.value_metric} | financial_rating=${l4.financial_rating} | impact_order=${l4.impact_order}`)
        .join("\n");
