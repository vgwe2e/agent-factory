...
describe("filterTopN", () => {
    it("selects correct top-3 from 10 candidates", () => {
        const candidates = Array.from({ length: 10 }, (_, i) => makeResult({ l4Id: `l4-${i}`, composite: (10 - i) * 0.1 }));
        const result = filterTopN(candidates, 3);
        assert.equal(result.survivors.length, 3);
        assert.deepEqual(result.survivors.map((s) => s.l4Id), ["l4-0", "l4-1", "l4-2"]);
    ...
    it("includes tied candidates at boundary within 1.1x cap", () => {
        ...
        const result = filterTopN(candidates, 3);
        assert.equal(result.survivors.length, 3);
        assert.deepEqual(result.survivors.map((s) => s.l4Id), ["l4-a", "l4-b", "l4-c"]);
    ...
    it("includes all ties when within 1.1x cap", () => {
        ...
        const candidates = [
            ...Array.from({ length: 9 }, (_, i) => makeResult({ l4Id: `l4-${i}`, composite: 1.0 - i * 0.05 })),
    ...
    it("caps at floor(topN * 1.1) when many tied candidates exceed limit", () => {
        ...
        const result = filterTopN(candidates, 3);
        assert.equal(result.survivors.length, 3);
        assert.deepEqual(result.survivors.map((s) => s.l4Id), ["l4-top1", "l4-top2", "l4-t1"]);
    ...
    it("breaks ties by aggregatedMaxValue DESC", () => {
        ...
        assert.deepEqual(result.survivors.map((s) => s.l4Id), ["l4-b", "l4-c"]);
    });
    ...
    it("breaks ties by l4Id ascending when composite and maxValue match", () => {
        ...
        assert.deepEqual(result.survivors.map((s) => s.l4Id), ["l4-a", "l4-m"]);
    });
    ...
    it("excludes eliminated candidates from ranking but includes in result", () => {
        ...
        assert.deepEqual(result.survivors.map((s) => s.l4Id), ["l4-a", "l4-b"]);
    ...
    it("reports accurate FilterStats", () => {
        const candidates = Array.from({ length: 20 }, (_, i) => makeResult({ l4Id: `l4-${String(i).padStart(2, "0")}`, composite: (20 - i) * 0.05 }));
