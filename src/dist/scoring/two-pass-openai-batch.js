...
import { buildAdoptionLensFromDeterministic, buildTechnicalLensFromLLM, buildValueLensFromDeterministic, computeTwoPassComposite, } from "./consolidated-scorer.js";
import { extractChatCompletionContent, normalizeOpenAiJsonSchema, runOpenAiBatch, stripNullFields, } from "../infra/openai-batch-client.js";
export async function runTwoPassScoringOpenAiBatch(args) {
    const scored = [];
    const errors = [];
    let scoredCount = 0;
    let promotedCount = 0;
    ...
    for (const preScore of filterResult.survivors) {
        ...
        const { composite, promotedToSimulation } = computeTwoPassComposite(preScore.composite, llmOutput.platform_fit.score / 3, llmOutput.sanity_verdict);
        const result = {
            skillId: skill.id,
...
async function writePreScoreTsv(outputDir, filterResult) {
...
function findSkillForPreScore(preScore, skillByL4, hierarchy) {
