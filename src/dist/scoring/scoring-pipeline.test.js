...
function makeMultiLensChatFn(options) {
    return async (messages, _format) => {
        ...
        if (options?.failForSkill && userMsg.includes(options.failForSkill)) {
            return { success: false, error: "Simulated LLM failure" };
        }
        ...
        return { success: false, error: "Unknown lens" };
    };
}
...
describe("scoreOneSkill", () => {
    it("produces complete ScoringResult with all fields", async () => {
        ...
        const result = await scoreOneSkill(skill, company, { components: "test", processBuilder: "test" }, makeMultiLensChatFn());
    ...
    it("skill with high scores has promotedToSimulation = true", async () => {
        const skill = makeSkill();
        const company = makeCompany();
        const result = await scoreOneSkill(skill, company, { components: "test", processBuilder: "test" }, makeHighScoringChatFn());
    ...
    it("skill with low scores has promotedToSimulation = false", async () => {
        const skill = makeSkill();
        const company = makeCompany();
        const result = await scoreOneSkill(skill, company, { components: "test", processBuilder: "test" }, makeLowScoringChatFn());
        assert.ok("composite" in result);
        const sr = result;
        assert.ok(sr.composite < 0.60, `Expected composite < 0.60, got ${sr.composite}`);
    ...
    it("LLM failure yields error with skillId, does not throw", async () => {
        ...
        const result = await scoreOneSkill(skill, company, { components: "test", processBuilder: "test" }, failChatFn);
        assert.ok("error" in result, "Result should be an error");
        assert.equal(result.skillId, "skill-bad");
        assert.equal(result.skillName, "Bad Skill");
    });
});
describe("scoreSkills", () => {
    it("scores multiple skills via generator", async () => {
        ...
        const firstSkillId = "skillId" in results[0] ? results[0].skillId : "";
        const secondSkillId = "skillId" in results[1] ? results[1].skillId : "";
    ...
    it("continues on failure for one skill", async () => {
        ...
        assert.equal(results[1].skillId, "s-bad");
    });
});
