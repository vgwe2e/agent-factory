...
describe("l4ActivitySchema", () => {
    it("rejects invalid financial_rating enum value", () => {
        ...
        try {
            ...
            assert.ok(err instanceof ZodError);
            const issue = err.issues[0];
            assert.ok(issue.path.includes("financial_rating"), `Expected path to contain 'financial_rating', got: ${JSON.stringify(issue.path)}`);
        }
    });
...
describe("l3OpportunitySchema", () => {
    it("rejects missing lead_archetype", () => {
        const { lead_archetype, ...invalid } = validL3;
        try {
            ...
            assert.ok(err instanceof ZodError);
            const issue = err.issues[0];
            assert.ok(issue.path.includes("lead_archetype"), `Expected path to contain 'lead_archetype', got: ${JSON.stringify(issue.path)}`);
        }
    });
