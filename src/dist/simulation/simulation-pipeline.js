 ...
 */
export async function runSimulationPipeline(inputs, outputDir, ollamaUrl, deps = DEFAULT_DEPS) {
    if (inputs.length === 0) {
        return {
            results: [],
            totalSimulated: 0,
    ...
    for (let i = 0; i < sorted.length; i++) {
        ...
        const defaultComponentMap = { streams: [], cortex: [], process_builder: [], agent_teams: [], ui: [] };
        ...
        const defaultIntegrationSurface = {
            source_systems: [],
            aera_ingestion: [],
            processing: [],
            ui_surface: [],
