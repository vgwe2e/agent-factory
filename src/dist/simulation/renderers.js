...
export function renderScenarioArtifacts(input, scenarioSpec, knowledgeIndex) {
...
function deriveComponentMap(spec) {
    const map = {
        streams: [],
        cortex: [],
        process_builder: [],
...
function deriveMockTest(input, spec, componentMap) {
    return {
...
function deriveIntegrationSurface(spec, componentMap) {
    return {
...
function renderStepNode(nodeId, step) {
...
function buildEntry(step, section) {
    const base = {
        name: step.component,
...
function collectAffectedComponents(componentMap) {
    return [
...
function extractFinancialContext(input) {
...
function inferGenericStreamName(preferredStreamType, inputName) {
    const normalizedType = normalizeStreamType(preferredStreamType);
    if (normalizedType !== "tbd") {
        return normalizedType;
    }
    const lower = inputName.toLowerCase();
    if (lower.includes("master") || lower.includes("reference"))
...
function pushUnique(list, candidate, keyFn) {
