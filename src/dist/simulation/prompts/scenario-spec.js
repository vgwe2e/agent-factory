 ...
 */
export function buildScenarioSpecPrompt(input, pbNodeNames, uiComponentNames, integrationPatternNames, capabilitiesContext, outputFormat = "yaml") {
    ...
    const l4Details = input.l4s
        ...
        if (l4.decision_articulation)
            parts.push(`  Decision: ${l4.decision_articulation}`);
        if (l4.description)
            parts.push(`  Description: ${l4.description}`);
        return parts.join("\n");
    })
        .join("\n");
