...
describe("generateIntegrationSurface", () => {
    ...
    it("returns success with valid YAML containing identified and tbd sources", async () => {
        globalThis.fetch = mock.fn(async () => new Response(makeOllamaResponse(VALID_INTEGRATION_SURFACE_YAML), { status: 200 }));
    ...
    it("strips code fences from YAML output", async () => {
        globalThis.fetch = mock.fn(async () => new Response(makeOllamaResponse(VALID_YAML_IN_FENCES), { status: 200 }));
        const result = await generateIntegrationSurface(mockInput, "http://localhost:11434");
        assert.equal(result.success, true);
        if (!result.success)
            return;
    ...
    it("returns error after 3 failed attempts", async () => {
        globalThis.fetch = mock.fn(async () => new Response(makeOllamaResponse(INVALID_YAML), { status: 200 }));
        const result = await generateIntegrationSurface(mockInput, "http://localhost:11434");
