...
import { generateSimulationText, resolveSimulationLlmConfig, } from "./llm-client.js";
describe("simulation llm client", () => {
    ...
    it("calls a vLLM server via /v1/chat/completions with auth", async () => {
        ...
        const result = await generateSimulationText([{ role: "user", content: "Return YAML" }], {
            backend: "vllm",
    ...
    it("calls Ollama via /api/chat when passed a legacy base URL string", async () => {
        ...
        const result = await generateSimulationText([{ role: "user", content: "Return Mermaid" }], "http://localhost:11434", { timeoutMs: 1000 });
        assert.equal(result.success, true);
