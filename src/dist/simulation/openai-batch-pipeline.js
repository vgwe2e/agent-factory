...
export async function runOpenAiBatchSimulation(inputs, outputDir, batchConfig, deps = DEFAULT_DEPS) {
    if (inputs.length === 0) {
        return {
            results: [],
            totalSimulated: 0,
    ...
    const requests = sorted.map((input) => {
        ...
        const messages = buildScenarioSpecPrompt(input, pbNodeNames, uiComponentNames, integrationPatternNames, knowledgeContext.capabilities, "json");
        return {
            customId,
            body: {
...
function getSimulationId(input) {
    return input.l4Activity?.id ?? input.opportunity?.l3_name ?? "unknown";
}
function buildFailedResult(subjectName, slug, reason) {
    return {
        l3Name: subjectName,
        slug,
        assessment: {
...
function emptyIntegrationSurface() {
    return {
        source_systems: [],
        aera_ingestion: [],
        processing: [],
