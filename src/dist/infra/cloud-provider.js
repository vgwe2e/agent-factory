...
export function createCloudProvider(config) {
    const { apiKey, gpuType = DEFAULT_GPU_TYPE, model = DEFAULT_MODEL, templateId = DEFAULT_TEMPLATE_ID, networkVolumeId, maxProvisionTimeoutMs = DEFAULT_PROVISION_TIMEOUT_MS, maxHealthTimeoutMs = DEFAULT_HEALTH_TIMEOUT_MS, pollIntervalMs = DEFAULT_POLL_INTERVAL_MS, } = config;
    // ---- GraphQL helper ---------------------------------------------------
    ...
    async function pollEndpointHealth(endpointId) {
        try {
    ...
    async function autoTeardown(endpointId) {
        try {
    ...
    async function provision() {
        if (!apiKey) {
            throw new Error("RUNPOD_API_KEY is required. Set it in CloudProviderConfig.apiKey or the RUNPOD_API_KEY environment variable.");
        }
        let endpointId;
        try {
      ...
      `;
            ...
            if (createResult.errors?.length) {
                throw new Error(`Failed to create RunPod endpoint: ${createResult.errors.map((e) => e.message).join(", ")}`);
            }
            ...
            while (Date.now() - provisionStart < maxProvisionTimeoutMs) {
                ...
                if (Date.now() - provisionStart + interval >= maxProvisionTimeoutMs) {
                    throw new Error(`RunPod endpoint ${endpointId} provision timed out after ${maxProvisionTimeoutMs}ms. ` +
            ...
            try {
                ...
                if (diagResp.ok) {
                    ...
                    if (loadedIds.length > 0) {
                        throw new Error(`Expected model '${model}' but loaded: [${loadedIds.join(", ")}]. ` +
                            `Use --vllm-url <url> to connect to a manually created pod.`);
                    }
                }
            }
            ...
            throw new Error(`vLLM health check timed out after ${maxHealthTimeoutMs / 1000}s -- model may not be loaded on endpoint ${endpointId}.${modelDiag} ` +
    ...
    async function teardown(endpoint) {
        try {
            const deleteMutation = `
        mutation {
