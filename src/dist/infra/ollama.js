...
export async function checkOllama(requiredModels = DEFAULT_REQUIRED_MODELS) {
    try {
        ...
        const missingModels = requiredModels.filter((required) => !availableNames.some((available) => available.startsWith(required)));
 ...
 */
export function formatOllamaStatus(status) {
    if (!status.connected) {
        return `${RED}Ollama: NOT CONNECTED -- ${status.error}${RESET}`;
    }
    const lines = [];
    if (status.missingModels.length === 0) {
        lines.push(`${GREEN}Ollama: Connected (${status.models.length} models available)${RESET}`);
    }
    else {
        lines.push(`${YELLOW}Ollama: Connected but missing models: ${status.missingModels.join(", ")}${RESET}`);
    }
