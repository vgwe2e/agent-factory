...
import { loadCheckpoint, saveCheckpoint, getCompletedNames, checkpointPath, createCheckpointWriter, clearCheckpointErrors, CHECKPOINT_FILENAME, CheckpointV2Schema, loadCheckpointForMode, getCompletedL4Ids, createCheckpointV2Writer, } from './checkpoint.js';
describe('checkpoint', () => {
    let tempDir;
    beforeEach(() => {
    ...
    it('loadCheckpoint falls back to .bak file when primary is corrupt', () => {
        ...
        writeFileSync(join(tempDir, CHECKPOINT_FILENAME + '.bak'), JSON.stringify(checkpoint, null, 2));
        const loaded = loadCheckpoint(tempDir);
        assert.ok(loaded);
...
describe('createCheckpointWriter', () => {
    ...
    function makeEntry(name, status = 'scored') {
...
describe('clearCheckpointErrors', () => {
    ...
    function makeCheckpointWith(entries) {
        return {
            version: 1,
            inputFile: '/test/input.json',
            startedAt: '2026-03-11T10:00:00Z',
