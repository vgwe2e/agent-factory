...
describe("backend-factory", () => {
    ...
    it("createBackend('vllm') with user-managed URL does not forward RUNPOD auth by default", async () => {
        ...
        const result = await config.chatFn([{ role: "user", content: "test" }], { type: "object", properties: {} });
    ...
    it("createBackend('vllm') throws when neither vllmUrl nor runpodApiKey provided", async () => {
        await assert.rejects(() => createBackend("vllm"), (err) => {
            assert.ok(err.message.includes("--vllm-url") && err.message.includes("RUNPOD_API_KEY"), `Error mentions both options: ${err.message}`);
            return true;
        });
    });
    it("createBackend('vllm') throws when vllmUrl is empty and no runpodApiKey", async () => {
        await assert.rejects(() => createBackend("vllm", { vllmUrl: "" }), (err) => {
            assert.ok(err.message.includes("--vllm-url") || err.message.includes("RUNPOD_API_KEY"), `Error mentions options: ${err.message}`);
            return true;
        });
    });
    ...
    it("createBackend('openai-batch') throws when OPENAI_API_KEY is missing", async () => {
        await assert.rejects(() => createBackend("openai-batch"), (err) => {
    ...
    it("createBackend throws for invalid backend string", async () => {
        await assert.rejects(() => createBackend("invalid-backend"), (err) => {
    ...
    it("createBackend('vllm') with runpodApiKey provisions a dedicated pod with explicit image and start command", async () => {
        ...
        assert.deepStrictEqual(dockerStartCmd.slice(0, 8), ["--model", DEFAULT_MODEL, "--max-model-len", "16384", "--dtype", "auto", "--api-key", vllmApiKey]);
