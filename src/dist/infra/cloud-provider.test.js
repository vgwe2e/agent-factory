...
describe("CloudProvider", () => {
    ...
    it("provision() returns endpointId and baseUrl on success", async () => {
        ...
        assert.ok(endpoint.baseUrl.startsWith("https://api.runpod.ai/v2/"), `Expected RunPod proxy URL with runpod.ai, got: ${endpoint.baseUrl}`);
        assert.ok(endpoint.provisionedAt instanceof Date);
        // PROV-01: GraphQL mutation must use dockerArgs, not env/MODEL_NAME
        assert.ok(capturedGraphqlBody.includes("dockerArgs"), `Expected GraphQL body to contain 'dockerArgs', got: ${capturedGraphqlBody.slice(0, 200)}`);
        assert.ok(capturedGraphqlBody.includes("--model"), `Expected GraphQL body to contain '--model', got: ${capturedGraphqlBody.slice(0, 200)}`);
        assert.ok(!capturedGraphqlBody.includes("MODEL_NAME"), `Expected GraphQL body NOT to contain 'MODEL_NAME', got: ${capturedGraphqlBody.slice(0, 200)}`);
    });
    ...
    it("healthCheck() returns true when /v1/models contains requested model (case-insensitive)", async () => {
        globalThis.fetch = (async () => jsonResponse({ data: [{ id: "Qwen/Qwen2.5-32B-Instruct" }] }));
    ...
    it("healthCheck() returns false when /v1/models has wrong model", async () => {
        globalThis.fetch = (async () => jsonResponse({ data: [{ id: "meta-llama/Llama-3-8B" }] }));
        const provider = createCloudProvider({ apiKey: "test-key", model: "Qwen/Qwen2.5-32B-Instruct" });
        const endpoint = {
            endpointId: "ep-1",
            baseUrl: "https://api.runpod.ai/v2/ep-1/openai/v1",
    ...
    it("teardown() succeeds", async () => {
        globalThis.fetch = (async () => jsonResponse({ data: { deleteEndpoint: null } }));
        const provider = createCloudProvider({ apiKey: "test-key" });
        const endpoint = {
            endpointId: "ep-1",
            baseUrl: "https://api.runpod.ai/v2/ep-1/openai/v1",
    ...
    it("provision() includes networkVolumeId in saveEndpoint mutation when config.networkVolumeId is provided", async () => {
        ...
        const endpoint = await provider.provision();
        assert.equal(endpoint.endpointId, endpointId);
        assert.ok(capturedGraphqlBody.includes("vol_abc123"), `Expected mutation body to include 'vol_abc123', got: ${capturedGraphqlBody.slice(0, 300)}`);
        assert.ok(capturedGraphqlBody.includes("networkVolumeId"), `Expected mutation body to include 'networkVolumeId', got: ${capturedGraphqlBody.slice(0, 300)}`);
    });
    ...
    it("provision() does NOT include networkVolumeId in mutation when config.networkVolumeId is undefined", async () => {
        ...
        const endpoint = await provider.provision();
        assert.equal(endpoint.endpointId, endpointId);
        assert.ok(!capturedGraphqlBody.includes("networkVolumeId"), `Expected mutation body NOT to include 'networkVolumeId', got: ${capturedGraphqlBody.slice(0, 300)}`);
    });
    it("teardown() is idempotent -- calling twice does not throw", async () => {
        ...
        globalThis.fetch = (async () => {
            ...
            if (callCount > 1) {
                ...
                return jsonResponse({ errors: [{ message: "Endpoint not found" }] }, 200);
            }
            return jsonResponse({ data: { deleteEndpoint: null } });
        });
        const provider = createCloudProvider({ apiKey: "test-key" });
