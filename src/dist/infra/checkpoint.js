...
export function checkpointPath(outputDir) {
...
export function saveCheckpoint(outputDir, checkpoint) {
    mkdirSync(outputDir, { recursive: true });
    const filePath = checkpointPath(outputDir);
    writeFileSync(filePath, JSON.stringify(checkpoint, null, 2), 'utf-8');
}
export function getCompletedNames(checkpoint) {
    if (!checkpoint)
        return new Set();
    return new Set(checkpoint.entries
 ...
 */
export function clearCheckpointErrors(outputDir) {
 ...
 */
export function loadCheckpointForMode(outputDir, scoringMode) {
    const filePath = checkpointPath(outputDir);
    if (!existsSync(filePath)) {
 ...
 */
export function getCompletedL4Ids(checkpoint) {
    if (!checkpoint)
        return new Set();
    return new Set(checkpoint.entries
        .filter((e) => e.status !== 'error')
 ...
 */
export function createCheckpointV2Writer(outputDir, checkpoint) {
    ...
    function atomicWrite() {
        try {
            ...
            try {
                mkdirSync(resolvedDir, { recursive: true });
                writeFileSync(checkpointPath(resolvedDir), JSON.stringify(checkpoint, null, 2), 'utf-8');
            }
            catch {
    ...
    function installSignalHandlers() {
        ...
        const unhandledHandler = (_reason) => {
 ...
 */
export function createCheckpointWriter(outputDir, checkpoint) {
