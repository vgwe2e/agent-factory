...
// ---------------------------------------------------------------------------
export function resolveOutputDir(explicitOutputDir, backend) {
...
program
    ...
    .option("--log-level <level>", `Structured log level (${LOG_LEVELS.join(", ")})`, "info")
    .option("--output-dir <path>", "Output directory for evaluation results")
    .addOption(new Option("--backend <backend>", "Scoring backend (ollama, vllm, or openai-batch)")
    .choices(["ollama", "vllm", "openai-batch"])
    .default("ollama"))
    .option("--vllm-url <url>", "vLLM server URL (required when --backend vllm without RUNPOD_API_KEY)")
    ...
    .option("--max-tier <n>", "Only score opportunities up to this tier (1, 2, or 3)", "3")
    ...
    .option("--top-n <n>", "Number of top-scoring L4 candidates to pass to LLM scoring (default: 50)", "50")
    .addOption(new Option("--scoring-mode <mode>", "Scoring pipeline mode")
    ...
    // Format revenue
    const revenueStr = company_context.annual_revenue !== null
        ? "$" +
            new Intl.NumberFormat("en-US").format(company_context.annual_revenue)
        : "N/A";
    // Format employee count
    const employeeStr = company_context.employee_count !== null
    ...
    console.log(`ERP Stack:     ${company_context.enterprise_applications.join(", ")}`);
    console.log();
    // Count total skills
    ...
    const pipelineFn = async () => {
        ...
        retryAttempt++;
        return runPipeline(opts.input, { ...pipelineOptions, concurrency: currentConcurrency }, logger);
    };
