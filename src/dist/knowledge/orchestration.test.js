...
import { getRouteForArchetype, getDecisionCriteria, getAllScenarios, getScenariosByRoute, getIntegrationPatterns, getIntegrationPattern, matchCriteria, getDecisionGuide, } from "./orchestration.js";
describe("orchestration query module", () => {
    ...
    describe("archetype-hierarchy type alignment", () => {
        it("all three LeadArchetype values have mappings", () => {
            const archetypes = ["DETERMINISTIC", "AGENTIC", "GENERATIVE"];
            for (const a of archetypes) {
                const mapping = getRouteForArchetype(a);
                assert.ok(mapping, `Missing mapping for ${a}`);
                assert.ok(["process", "agentic_ai", "hybrid"].includes(mapping.primary_route), `Invalid primary_route for ${a}`);
            }
        });
    });
});
