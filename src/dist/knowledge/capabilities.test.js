...
import { getAllCapabilities, getAllCapabilitiesFlat, getUseCaseMappings, getCapabilityKeywords, } from "./capabilities.js";
describe("getAllCapabilities", () => {
    ...
    it("each pillar has at least 1 capability", () => {
        for (const pillar of pillars) {
            assert.ok(pillar.capabilities.length >= 1, `${pillar.name} should have at least 1 capability`);
        }
    });
    it("each capability has required fields", () => {
        for (const pillar of pillars) {
            for (const cap of pillar.capabilities) {
                ...
                assert.ok(Array.isArray(cap.best_for) && cap.best_for.length > 0, `${cap.name} should have non-empty best_for array`);
                assert.ok(Array.isArray(cap.keywords) && cap.keywords.length > 0, `${cap.name} should have non-empty keywords array`);
            }
        }
    });
});
...
describe("getUseCaseMappings", () => {
    ...
    it("returns at least 15 use case mappings", () => {
        assert.ok(mappings.length >= 15, `Expected at least 15 mappings, got ${mappings.length}`);
    });
    it("each mapping has required fields", () => {
        for (const m of mappings) {
            assert.ok(m.use_case, "mapping should have use_case");
            assert.ok(Array.isArray(m.primary_components) && m.primary_components.length > 0, `${m.use_case} should have non-empty primary_components`);
            assert.ok(Array.isArray(m.supporting_components), `${m.use_case} should have supporting_components array`);
            assert.ok(["AI/ML", "Rule-Based", "Hybrid"].includes(m.skill_type), `${m.use_case} should have valid skill_type, got ${m.skill_type}`);
            assert.ok(Array.isArray(m.keywords) && m.keywords.length > 0, `${m.use_case} should have non-empty keywords`);
        }
    });
...
describe("getCapabilityKeywords", () => {
    ...
    it("each classification has keywords and components", () => {
        for (const [key, classification] of Object.entries(kw)) {
            assert.ok(Array.isArray(classification.keywords) && classification.keywords.length > 0, `${key} should have non-empty keywords`);
            assert.ok(Array.isArray(classification.components) && classification.components.length > 0, `${key} should have non-empty components`);
        }
    });
    ...
    it("ai_ml components include Cortex Auto Forecast", () => {
        assert.ok(kw.ai_ml.components.includes("Cortex Auto Forecast"), "AI/ML components should include Cortex Auto Forecast");
    });
});
