...
describe("getPBNodesByCategory", () => {
  ...
  it("returns empty array for invalid category", () => {
    ...
    const nodes = getPBNodesByCategory("nonexistent" as "core");
...
describe("getWorkflowPattern", () => {
  it("returns ETL Pattern with correct node_sequence", () => {
    ...
    assert.ok(
      pattern.node_sequence.includes("Data View"),
      "ETL Pattern should include Data View"
    );
  });
...
describe("referential integrity", () => {
  it("every node in every pattern exists in getAllPBNodes", () => {
    const allNodeNames = new Set(
      getAllPBNodes().map((n) => n.name.toLowerCase())
    );
    const patterns = getWorkflowPatterns();
    ...
    for (const pattern of patterns) {
      for (const nodeName of pattern.node_sequence) {
        assert.ok(
          allNodeNames.has(nodeName.toLowerCase()),
          `Pattern "${pattern.name}" references node "${nodeName}" which does not exist in getAllPBNodes()`
        );
      }
    }
  });
