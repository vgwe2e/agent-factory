...
export function createPodProvider(config: PodProviderConfig): PodProvider {
  const {
    apiKey,
  ...
  async function getPod(podId: string): Promise<GetPodResponse> {
    return rest<GetPodResponse>(`/pods/${podId}`, {
      method: "GET",
    });
  }

  function buildBaseUrl(podId: string): string {
    return `https://${podId}-${DEFAULT_PORT}.proxy.runpod.net/v1`;
  }

  async function healthCheck(pod: ProvisionedPod): Promise<boolean> {
    try {
  ...
  async function teardown(pod: ProvisionedPod): Promise<void> {
    try {
  ...
  async function provision(): Promise<ProvisionedPod> {
    if (!apiKey) {
      throw new Error(
        "RUNPOD_API_KEY is required. Set it in PodProviderConfig.apiKey or the RUNPOD_API_KEY environment variable.",
      );
    }
    ...
    try {
      ...
      const created = await rest<CreatePodResponse>("/pods", {
        method: "POST",
      ...
      if (
        (podState.uptimeSeconds ?? 0) === 0 &&
        podState.machineId == null &&
        podState.publicIp == null
      ) {
        throw new Error(
          `RunPod pod ${podId} did not finish container startup after ${maxProvisionTimeoutMs}ms. ` +
      ...
      throw new Error(
        `vLLM health check timed out after ${maxHealthTimeoutMs / 1000}s on pod ${podId}. ` +
