...
import type { ResilientResult } from "./retry-policy.js";
...
describe("callWithResilience", () => {
  it("returns primary result with resolvedVia='primary' when primaryCall succeeds", async () => {
    ...
    const result: ResilientResult<Score> = await callWithResilience({
      primaryCall,
      schema: ScoreSchema,
      label: "test-primary",
      maxRetries: 1,
  ...
  it("falls back to fallbackCall with resolvedVia='fallback' when primary exhausts retries", async () => {
    const primaryCall = async (): Promise<string> => {
  ...
  it("returns resolvedVia='skipped' with success=false when both primary and fallback fail", async () => {
    ...
    const fallbackCall = async (): Promise<string> => {
  ...
  it("skipReason contains the original error message from primary failure", async () => {
    ...
    assert.ok(
      result.skipReason.includes("LLM timeout after 120s"),
      `Expected skipReason to contain 'LLM timeout after 120s', got: ${result.skipReason}`,
    );
  });
});
