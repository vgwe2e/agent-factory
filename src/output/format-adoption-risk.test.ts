...
describe("formatAdoptionRisk", () => {
  ...
  it("shows 'None identified.' for empty flag type sections", () => {
    ...
    for (const section of sections) {
      assert.ok(
        section.includes("None identified."),
        `Section should show 'None identified.' but got: ${section.slice(0, 100)}`,
      );
    }
  });

  it("groups flagged opportunities by flag type", () => {
    const opps: TriageResult[] = [
      makeTriage({
        l3Name: "Dead Opp",
        l1Name: "Finance",
        l2Name: "AP",
  ...
  it("marks skipped opportunities", () => {
    ...
    assert.ok(
      result.includes("SKIP") || result.includes("skipped") || result.includes("Skipped"),
      "Should indicate skipped status",
    );
  });
