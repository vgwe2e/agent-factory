...
describe("buildKnowledgeIndex", () => {
  it("includes all 22 PB nodes, 21 UI components, 7 workflow patterns, 4 integration patterns", () => {
    ...
    for (const value of index.values()) {
      if (value.startsWith("PB:")) pbCount++;
      else if (value.startsWith("UI:")) uiCount++;
      else if (value.startsWith("Pattern:")) patternCount++;
      else if (value.startsWith("Integration:")) integrationCount++;
    }
