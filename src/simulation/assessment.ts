...
export function assessSimulation({
  ...
  inferredCount,
  mermaidValid,
}: AssessmentInput): SimulationAssessment {
  ...
  const ambiguityRiskScore = toPercent(clamp01(
    (inferredRatio * 0.5) +
    (tbdRatio * 0.3) +
    (mermaidValid ? 0 : 0.2),
  ));
  ...
  const implementationReadinessScore = clampScore(
    Math.round(
...
export function countAssessmentVerdicts(
  assessments: Array<SimulationAssessment | undefined>,
): Record<SimulationFilterVerdict, number> {
  const counts: Record<SimulationFilterVerdict, number> = {
    ADVANCE: 0,
    REVIEW: 0,
    HOLD: 0,
  };
...
function decideVerdict(
  scores: Pick<
    SimulationAssessment,
    "groundednessScore" | "integrationConfidenceScore" | "ambiguityRiskScore" | "implementationReadinessScore"
  >,
): SimulationFilterVerdict {
  if (
    scores.implementationReadinessScore >= 70 &&
  ...
  if (
    scores.implementationReadinessScore >= 45 &&
