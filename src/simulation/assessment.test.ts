...
import type { MockTest, ScenarioSpec, IntegrationSurface, SimulationAssessment } from "../types/simulation.js";
...
function makeMockTest(overrides: Partial<MockTest> = {}): MockTest {
  return {
...
function makeIntegrationSurface(overrides: Partial<IntegrationSurface> = {}): IntegrationSurface {
  return {
    source_systems: [{ name: "SAP S/4HANA", status: "identified", type: "ERP" }],
...
describe("countAssessmentVerdicts", () => {
  it("counts verdict distribution", () => {
    const counts = countAssessmentVerdicts([
      { verdict: "ADVANCE" } as SimulationAssessment,
      { verdict: "ADVANCE" } as SimulationAssessment,
      { verdict: "REVIEW" } as SimulationAssessment,
      { verdict: "HOLD" } as SimulationAssessment,
      undefined,
    ]);
