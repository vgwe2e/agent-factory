...
describe("generateComponentMap", () => {
  ...
  it("returns success with known PB/UI names set to confirmed", async () => {
    globalThis.fetch = mock.fn(async () => makeOllamaResponse(VALID_YAML_KNOWN_NAMES)) as unknown as typeof fetch;
  ...
  it("returns success with unknown names set to inferred", async () => {
    globalThis.fetch = mock.fn(async () => makeOllamaResponse(VALID_YAML_UNKNOWN_NAMES)) as unknown as typeof fetch;
  ...
  it("strips code fences from YAML output", async () => {
    globalThis.fetch = mock.fn(async () => makeOllamaResponse(VALID_YAML_WITH_FENCES)) as unknown as typeof fetch;
  ...
  it("returns failure after 3 consecutive YAML parse failures", async () => {
    globalThis.fetch = mock.fn(async () => makeOllamaResponse(INVALID_YAML)) as unknown as typeof fetch;
...
describe("buildComponentMapPrompt", () => {
  it("includes PB node names, UI component names, and integration patterns", async () => {
    ...
    const messages = buildComponentMapPrompt(
      input,
      ["Interface", "If", "Action Item"],
      ["Dashboard", "Table", "Chart"],
      ["ETL Pattern", "API Gateway"],
    );
