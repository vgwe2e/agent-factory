...
export type ScenarioSpecResult =
  | { success: true; data: { scenarioSpec: ScenarioSpec; attempts: number } }
...
export async function generateScenarioSpec(
  ...
  const baseMessages = buildScenarioSpecPrompt(
    input,
    pbNodeNames,
    uiComponentNames,
    integrationPatternNames,
    knowledgeCtx.capabilities,
  );
  const errors: string[] = [];
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      if (parsed.success) {
        return {
          success: true,
          data: { scenarioSpec: parsed.data as ScenarioSpec, attempts: attempt },
        };
      }
